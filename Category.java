...
public class Category {
	 ...
	 * @throws IOException
	 */
	public void parseCategory(Map<String, Project> projectsMetadata, HttpServletResponse res)
			throws ParseException, RarException, IOException {
...
//				 String description = descriptionStr.split(":")[1];
	 ...
	 * @throws IOException
	 */
	private void parseDeliverables(Project pr, HttpServletResponse res)
			throws ParseException, RarException, IOException {
		PrintWriter out = res.getWriter();
		Document doc = null;
	 ...
	 * @throws IOException
	 */
	private void parseDelivery(Delivery d, ServletResponse res) throws ParseException, RarException, IOException {
		PrintWriter out = res.getWriter();
		Document doc = null;
		String url = d.getUrl();
		...
		for (Element fileEl : listFiles) {
			...
			if (d.getType().equalsIgnoreCase("document")) {
				...
				List<String> fileNames = downloadResource(fUrl, path, 3, this.formats, d.getDate(), res);
	 ...
	 */
	public void writeMetadata() throws FileNotFoundException {
...
//	}
	...
	private List<String> downloadResource(String urlStr, String outputFolder, int nTry, String[] allowedFormats,
			Date date, ServletResponse res) throws ParseException, RarException, IOException {
		PrintWriter out = res.getWriter();
					...
					try {
						...
						if (path.contains(".zip")) {
							...
							if (!folder.isDirectory()) {
								FileUtils.copyURLToFile(url, file, 30000, 30000);
								System.out.println(name + " - downloded at " + (i + 1) + " attempt");
								fileNames = unrar(path, directory);
								boolean deleted = file.delete();
	 ...
	 */
	private static List<String> unzip(String fileZipPath, String destDirPath) throws IOException {
		List<String> fileNames = new LinkedList<String>();
	...
	private static List<String> unrar(String fileRarPath, String destDirPath) throws RarException, IOException {
		List<String> fileNames = new LinkedList<String>();
		String rarName = getFileName(fileRarPath);
		File f = new File(fileRarPath);
			...
			if (!(rarNameNoExt.contains(folder.getName()))) {
				...
				while (listHeader != null) {
					...
					if (fileName.contains(".pdf")) {
						...
						destDir.mkdirs();
						Junrar.extract(fileRarPath, destDir.getAbsolutePath());
						fileNames.add(fileName);
					}
